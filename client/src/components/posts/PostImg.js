          ...
          <div>
            ...
            <form onSubmit= {this._handleSubmit}>
             ...
             <div className="flex flex-wrap justify-center mt-5">
               <div className="flex justify-center font-bold w-3/5 text-2xl text-purple mb-5"><p className="flex">Ready to Post?</p></div>
                <div className="flex justify-center w-3/5">
                <button className="font-bold lg:text-xl text-grey-lightest mr-4 py-2 px-4 bg-purple hover:bg-purple-light rounded" type="submit" onClick={this._handleSubmit}>Post</button>
                <button className="font-bold lg:text-xl text-grey-lightest py-2 px-4 bg-red hover:bg-red-light rounded" type="button" onClick={this.props.onCancel}>Cancel</button>
                </div>
             </div> : 
             <div>
              <div className="bg-orange-lightest border-l-4 border-orange text-orange-dark p-4" role="alert">
                ...
                <li>You can only upload <b>3</b> images for one post</li>
                <li>Only <b>jpeg</b> file accepted</li>
                </ul>
              </div>
              <div className="flex justify-start items-center">
                <label className="flex mt-3 bg-purple-light border-2 border-purple rounded-lg">
                  <p className="flex mx-2 p-1 font-bold font-italic text-grey-lightest">Choose Image Files</p>
                  <input className="upload" type="file" accept='image/jpeg' style={{opacity:0}} onChange={this._handleImageChange} />
                </label>
              </div>
              <Image 
    ...
    return (
      ...
      {this.props.imagesUrls.map((imageUrl, i) => (
         <div key={imageUrl+ i} className="my-2"> 
            <button className="text-red mr-2" key={imageUrl+ i+ ' div'} type='button' onClick={()=>{this.delete(imageUrl, i)}}>
              <FaTimes />
            </button>
            <img key={'img'+i} alt={'uploadImg: '+imageUrl +i } src={imageUrl} className="w-1/3 mr-3"/>
        </div>
    ))}
    </div>
    );
